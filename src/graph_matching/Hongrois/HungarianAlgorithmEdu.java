package graph_matching.Hongrois;
...
public class HungarianAlgorithmEdu {
	...
	public static SimpleMatrix getPositive(SimpleMatrix mat){
		double min=10000;
		SimpleMatrix resultat=mat;
		for(int i=0;i<mat.getMatrix().getNumCols();i++){
			for(int j=0;j<mat.getMatrix().getNumRows();j++){
				if(mat.get(j,i)<min){
					min=mat.get(j, i);
				}
			}
		}
		resultat=resultat.plus(Math.abs(min));
